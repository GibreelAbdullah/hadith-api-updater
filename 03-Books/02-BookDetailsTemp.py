    ...
    for hadith in collectionDetails["hadiths"]:
        try:
            if "minHadith" in bookList["books"][str(hadith["reference"]["book"])]:
                bookList["books"][str(hadith["reference"]["book"])]["minHadith"] = min(bookList["books"][str(hadith["reference"]["book"])]["minHadith"],hadith["arabicnumber"])
                bookList["books"][str(hadith["reference"]["book"])]["maxHadith"] = max(bookList["books"][str(hadith["reference"]["book"])]["minHadith"],hadith["arabicnumber"])
            else:
                bookList["books"][str(hadith["reference"]["book"])]["minHadith"] = hadith["arabicnumber"]
                bookList["books"][str(hadith["reference"]["book"])]["maxHadith"] = hadith["arabicnumber"]
        except:
            if "minHadith" in bookList["books"][str(hadith["reference"]["book"])]:
